  ...
  methods:{
    async handleSave(){
      if(this.user_nick.len != 0){
        ...
        await this.$axios.post(`${process.env.BACKEND_URL}/update/ucenter`,data);
        this.$message({
          message: "修改成功！",
          type: "success"
        });
    ...
    async reloadAvatar(){
      const result = await this.$axios.post(`${process.env.BACKEND_URL}/get/user_pic`,{user_name:this.$store.state.userName});
